...
#' @usage scrolling_spectro(wave, hop.size = 11.6, wl = NULL, 
...
scrolling_spectro <- function(wave, hop.size = 11.6, wl = NULL, ovlp = 50, flim = NULL, speed = 1, fps = 50, t.display = 1.5, fix.time = TRUE, res = 70, width = 700, height = 400, parallel = 1, pb = TRUE, play = TRUE, ...)
{

  # change speed
  if (speed != 1)
    wave@samp.rate <- wave@samp.rate * 0.5
  ...
  if (.Platform["OS.type"] == "windows") {
    ...
    system(paste(Sys.getenv("R_BROWSER"), "scroll.spectro.mp4"), ignore.stdout = TRUE, 
           ignore.stderr = TRUE)
  }
